...
struct StringDropdownField: NSViewRepresentable {
    var items: [String]
    @Binding var text: String
    var placeholder: String
    var keepSelectedValueInField: Bool = false
    var onSelect: (String) -> Void
    var onCommit: (String) -> Void

    @available(macOS 13.0, *)
    func sizeThatFits(_ proposal: ProposedViewSize, nsView: DropdownFieldContainerView, context: Context) -> CGSize? {
    ...
    func makeNSView(context: Context) -> DropdownFieldContainerView {
        let container = DropdownFieldContainerView()
        ...
        container.button.action = #selector(DropdownFieldCoordinator.toggleDropdown(_:))

        let click = NSClickGestureRecognizer(target: context.coordinator, action: #selector(DropdownFieldCoordinator.textFieldClicked(_:)))
    ...
    func updateNSView(_ container: DropdownFieldContainerView, context: Context) {
    ...
    func makeCoordinator() -> DropdownFieldCoordinator {
        DropdownFieldCoordinator(parent: self)
    }

    final class DropdownFieldContainerView: NSView {
    ...
    class DropdownFieldCoordinator: NSObject, NSTextFieldDelegate, NSTableViewDataSource, NSTableViewDelegate {
        var parent: StringDropdownField
        var allItems: [String] = []
        var filteredItems: [String] = []
        private weak var container: DropdownFieldContainerView?
        ...
        init(parent: StringDropdownField) {
            self.parent = parent
        }
        ...
        func attach(container: DropdownFieldContainerView) {
        ...
        func refilter(query: String) {
            ...
            if dropdownPanel?.isVisible == true, filteredItems.isEmpty {
                closeDropdown()
            } else {
        ...
        func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
            ...
            let identifier = NSUserInterfaceItemIdentifier("DropdownCell")
        ...
        private func commitSelection(_ value: String) {
            ...
            if parent.keepSelectedValueInField {
        ...
        private func commitFreeform(_ value: String) {
            ...
            if parent.keepSelectedValueInField {
                setValueAndClose(trimmed)
            } else {
                clearAndClose()
            }
        }

        private func setValueAndClose(_ value: String) {
            closeDropdown()
            DispatchQueue.main.async {
                self.container?.textField.stringValue = value
                self.parent.text = value
                self.refilter(query: value)
                DispatchQueue.main.async {
                    self.moveCursorToEndOfField()
                }
            }
        }

        private func moveCursorToEndOfField() {
            guard let container else { return }
            let text = container.textField.stringValue
            guard let editor = container.textField.currentEditor() as? NSTextView else { return }
